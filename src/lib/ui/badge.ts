...
export const BADGE_TAG = "bullshit-detector-badge";
...
export class Badge {
  ...
  #loading(): HTMLElement | null {
    ...
    return h(
      ...
      h("span", { class: "pill pill--loading" }, "🐂 Sniffing…"),
    );
  }
      ...
      parts.index
        ? h(
            "button",
            {
              ...
              title: "Bullshit Index. Click for the breakdown.",
              onclick: toggle,
            },
  ...
  #details(analysis: Analysis, index: number, provider?: ProviderId): HTMLElement {
    ...
    return h(
      ...
      legend && h("p", { class: "notice" }, "On the zero-BS list. Science can't explain it."),
      h(
        "ul",
        { class: "signals" },
      ...
      !legend &&
        ...
        h(
          "p",
          { class: "ai-line" },
          `${genre.emoji} ${genre.label} posts count as ${Math.round(genreWeight * 100)}% bullshit for you`,
        ),
      !legend &&
        h(
          "p",
...
function copyVerdict(button: HTMLButtonElement, index: number, legend: boolean): void {
  const verdict = legend ? LEGEND_VERDICT : verdictFor(index);
  const text = `This LinkedIn post scored ${index}% on the Bullshit Index ${verdict.emoji} (${verdict.label}). Measured with Bullshit Detector: ${REPO_URL}`;
