...
import { buildAnalysis, bullshitIndex, detectTropes, personalIndex } from "@/lib/analysis/scoring";
...
describe("bullshitIndex", () => {
  it("spans 0..100", () => {
    expect(bullshitIndex(all(0))).toBe(0);
    expect(bullshitIndex(all(1))).toBeGreaterThanOrEqual(95);
    expect(bullshitIndex(all(1))).toBeLessThanOrEqual(100);
  });

  it("goes up with every signal", () => {
    for (const id of SIGNAL_IDS) {
      expect(bullshitIndex({ ...all(0.3), [id]: 0.9 })).toBeGreaterThan(bullshitIndex(all(0.3)));
    }
  });

  it("ignores faint maybes, so a clean post stays clean", () => {
    const noisy = { ...all(0.1), buzzwords: 0, fluff: 0, self_promotion: 0 };
    expect(bullshitIndex(noisy)).toBe(0);
  });
  ...
  it("lets several strong tropes push a post near the top", () => {
    ...
    expect(bullshitIndex(cliche)).toBeGreaterThanOrEqual(85);
  });

  it("clamps out-of-range and non-finite values instead of trusting them", () => {
    expect(bullshitIndex({ ...all(0), buzzwords: 7, fluff: Number.NaN })).toBe(
      bullshitIndex({ ...all(0), buzzwords: 1 }),
    );
    expect(bullshitIndex(all(-3))).toBe(0);
  });
});
...
describe("verdicts", () => {
  it("covers every index with the matching tier", () => {
    ...
    expect(verdictFor(100).label).toBe("Certified BS");
...
describe("personalIndex", () => {
  ...
  it("makes a genre at 100% pure bullshit", () => {
